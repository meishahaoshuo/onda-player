...
const W = 1084
const H = 736
const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'onda-back2-'))
...
report.setup = await evaluate(`(async () => {
  ...
  await window.__musicTest.pl('create', '验证歌单')
  return { ok: true }
})()`)
...
const PROBE = `(() => {
  ...
  const rb = r(back), rx = r(box)
  return {
    backCount: document.querySelectorAll('.head-back').length,
    legacyCloseCount: document.querySelectorAll('.header-close').length,
    backDisabled: back ? back.disabled : null,
    backRect: rb,
    searchRect: rx,
    gap: rb && rx ? +(rx.left - rb.right).toFixed(1) : null,
    overlap: rb && rx ? rx.left < rb.right : null,
    backAboveSearch: rb && rx ? rb.top < rx.bottom && rb.bottom > rx.top : null,
  }
})()`

/* ---------- 顶层视图：按钮应在但置灰 ---------- */
report.topLevel = await evaluate(`(async () => {
...
report.topLevelProbe = await evaluate(PROBE)
report.topLevelClick = await evaluate(`(async () => {
  document.querySelector('.head-back').click()
  await new Promise((r) => setTimeout(r, 700))
  const { useUiStore } = await import('/src/stores/ui.ts')
  return { detailKeyAfterDisabledClick: useUiStore().detailKey, stillAlbums: useUiStore().activeView }
})()`)
await shot('40-top-level-back.png')

/* ---------- 聚焦态：搜索框只向右展，不得压住返回钮 ---------- */
report.focusProbe = await evaluate(`(async () => {
  ...
  const out = { searchLeft: Math.round(box.left), searchWidth: Math.round(box.width), backRight: Math.round(back.right), gap: Math.round(box.left - back.right), overlap: box.left < back.right }
  document.querySelector('.search-input').blur()
  return out
})()`)

/* ---------- 专辑详情 ---------- */
const open = (view, keyExpr) => `(async () => {
  const { useUiStore } = await import('/src/stores/ui.ts')
  ...
  ui.activeView = '${view}'
  ui.detailKey = ${keyExpr}
  await new Promise((r) => setTimeout(r, 1500))
  return { detailKey: ui.detailKey }
})()`

report.album = await evaluate(open('albums', 'lib.albums[0].key'))
report.albumProbe = await evaluate(PROBE)
await shot('41-album-detail-newback.png')
report.albumBack = await evaluate(`(async () => {
  document.querySelector('.head-back').click()
  await new Promise((r) => setTimeout(r, 1800))
  const { useUiStore } = await import('/src/stores/ui.ts')
  const ui = useUiStore()
  return { detailKey: ui.detailKey, layerGone: !document.querySelector('.detail-layer'), disabledNow: document.querySelector('.head-back').disabled }
})()`)

/* ---------- 艺术家详情 ---------- */
report.artist = await evaluate(open('artists', 'lib.artists[0].name'))
report.artistProbe = await evaluate(PROBE)
await shot('42-artist-detail-newback.png')
report.artistBack = await evaluate(`(async () => {
  document.querySelector('.head-back').click()
  await new Promise((r) => setTimeout(r, 1800))
  const { useUiStore } = await import('/src/stores/ui.ts')
  return { detailKey: useUiStore().detailKey }
})()`)

/* ---------- 歌单详情（此前重复按钮的元凶） ---------- */
report.playlist = await evaluate(open('playlists', 'pl.playlists[0]?.id'))
report.playlistProbe = await evaluate(PROBE)
await shot('43-playlist-detail-newback.png')
report.playlistBack = await evaluate(`(async () => {
  document.querySelector('.head-back').click()
  await new Promise((r) => setTimeout(r, 1800))
  const { useUiStore } = await import('/src/stores/ui.ts')
  return { detailKey: useUiStore().detailKey }

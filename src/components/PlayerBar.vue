...
/* 胶囊 + 液态玻璃：更强立体投影（玻璃更「厚」）。
   外框那一圈高光已去掉（用户 2026-09-15：「深色模式下的胶囊播放条外框的一层高光去掉」）。
   实测那圈高光由两条声明叠成（深色 + 液化玻璃，四缘峰值亮度）：
     ① `border: 1px solid rgba(255,255,255,.38)` → 上106 / 下107 / 左117 / 右109（贯通四边）
     ② `inset 0 1.5px 1px rgba(255,255,255,.8)` → 上205 / 左92 / 右81
        （pill 圆角让它沿轮廓绕了一圈，不只在顶缘）
   逐层隔离验证：单独关掉 ①，上下两条亮线消失；单独关掉 ②，顶缘与两侧的亮线消失。
   两条都摘掉后四缘回落到材质本色（约 35~50）。

   ⚠️ 边框写 `transparent` 而不是删掉：胶囊是 border-box，留 1px 占位可让内侧排布
   与圆角轮廓一像素都不动。浅色主题不改（仍是 rgba(0,0,0,0.1) 细线 + 白色顶高光）。 */

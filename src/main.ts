 ...
 * 一个入口，两种窗口。
 *
 * Rust 侧的 `tray-menu` 窗口以 `index.html?tray=1` 加载，这时只挂托盘浮层菜单
 * （见 `src/tray.ts`）—— **不能**跑启动过场、**不能**去定位/显示窗口、也不该注册 PWA。
 * 否则右键弹个菜单会连带触发一整套开机流程：`primeWindow` 会把浮层自己
 * 当成主窗口去定位和 show，菜单直接错位。
 */
const isTrayWindow = new URLSearchParams(location.search).get('tray') === '1'

if (isTrayWindow) {
  void import('./tray').then((m) => m.mountTrayMenu())
} else {
  /**
